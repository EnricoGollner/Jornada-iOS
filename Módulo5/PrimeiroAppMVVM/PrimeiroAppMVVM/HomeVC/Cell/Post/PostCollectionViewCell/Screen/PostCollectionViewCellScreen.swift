...
class PostCollectionViewCellScreen: UIView {
    ...
    @objc func tappedPostImageView() {
        ...
        UIView.animate(withDuration: 0.8, delay: 0, usingSpringWithDamping: 0.3, initialSpringVelocity: 0, options: .curveEaseIn, animations: {
            ...
            self.likeImageView.center.y += 50
        }, completion: { finished in  // Quando a animação for concluída - volte para os estados normais:
            UIView.animate(withDuration: 0.8, delay: 0, usingSpringWithDamping: 0.3, initialSpringVelocity: 0, options: .curveEaseIn, animations: {
                self.postImageView.transform = .identity  // imagem volta para o próprio tamanho.
                self.likeImageView.center.y -= 50  // o like fica em sua respectiva posição
    ...
    private func configConstraints() {
        NSLayoutConstraint.activate([
            ...
            userNameLabel.leadingAnchor.constraint(equalTo: profileImageView.trailingAnchor, constant: 10),  // A parte direita o userNameLabel, tem a distância de 10, em relação a parte esquerda da profileImageView (userNameLabel está a direita)
